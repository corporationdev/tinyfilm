...
interface PersistentEditHistoryStore {
...
interface PersistentEditHistory {
  loaded: boolean
  canUndo: boolean
  canRedo: boolean
  undoLabel: string | null
...
function snapshotEditHistoryState(state: EditHistoryState): EditHistorySnapshot {
...
export function createPersistentEditHistoryStore({
  ...
  now = Date.now,
  onChange
}: PersistentEditHistoryStoreOptions): PersistentEditHistoryStore {
  let state = initialState
  let queue = Promise.resolve()

  const save = async (nextState: EditHistoryState): Promise<void> => {
    state = nextState
    onChange(nextState)
    try {
  ...
  return {
    snapshot: (): EditHistorySnapshot => snapshotEditHistoryState(state),
...
export async function createPersistentEditHistoryController({
  ...
  now?: () => number
  onChange: (state: EditHistoryState) => void
}): Promise<PersistentEditHistoryStore> {
...
export function usePersistentEditHistory(
  options: UsePersistentEditHistoryOptions
): PersistentEditHistory {
  ...
  useEffect(() => {
    ...
    // Studio resets the loaded project history synchronously when the project id changes.
  ...
  const recordEdit = useCallback(async (input: RecordEditInput): Promise<void> => {
